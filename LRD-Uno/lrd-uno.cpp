...
void setup() {
  ...
  Serial.begin(9600); // ver os números no PC
}
...
void loop() {
  ...
  // MATEMÁTICA: Mapeia a leitura (ajustada pelo potenciômetro) 
  // para frequências musicais (Ex: 261Hz [Dó] a 523Hz [Dó oitava])
  // O valor 1023 é o máximo de luz, conforme você faz sombra, o valor cai.
  
  int frequencia = map(leituraLuz, 0, ajusteSensibilidade, 261, 523);

  // Filtro simples para não tocar som se houver muita luz (mão longe)
  ...
  delay(10); // Estabilidade do sistema
}
